...
function searching() {
	...
	document.fr.contents.value=document.fr.content.value;
	document.fr.submit();
}
</script>
<style type="text/css">
			...
			<div id="container">
				...
				<div id="skipCt">
					...
					<section id="containerBody">
						...
						<option value="member_num" <%if(select.equals("member_num")){ %>selected <%} %>>회원번호</option>
						<option value="member_id" <%if(select.equals("member_id")){ %>selected <%} %>>아이디</option>
						<option value="Krname" <%if(select.equals("Krname")){ %>selected <%} %>>이름</option>
						</select>
						<input type="text" name="content" value="<%=content %>" style="height: 18px; float: left;">
						<input type="image" id="btn_search" onclick="return searching()">
							...
							<table class="memberlist">
								...
								if(memberlist.size()>0){
									...
									%>
									<tr>
									<td colspan="5">해당 회원을 찾을 수 없습니다.</td>
									</tr>
									<%
								}
								%>
